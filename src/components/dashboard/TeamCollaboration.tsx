                  ...
                  <td className="py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full border border-gray-100 bg-gray-100 text-emerald-800 flex items-center justify-center font-bold text-sm">
                        {(activity.assignments?.[0]?.user?.name || activity.assignments?.[0]?.user?.email || "?").charAt(0).toUpperCase()}
                      </div>
                      <div>
                        <p className="font-medium text-sm text-gray-900 truncate max-w-[120px]">{activity.assignments?.[0]?.user?.name || activity.assignments?.[0]?.user?.email || "Sin asignar"}</p>
                        <p className="text-xs text-gray-400 truncate max-w-[120px]">{activity.project?.name || ""}</p>
                      </div>
                    </div>
                  </td>
